...
class RANK_IMIM_Random_Dataset_twostyle(data.Dataset):
    '''
    ...
    def name(self):
        return 'RANK_IMIM_Random_Dataset_twostyle'

    def __init__(self, opt,is_train):
        super(RANK_IMIM_Random_Dataset_twostyle, self).__init__()
        self.opt = opt
        ...
        # get full all img paths
        # if self.paths_img2 and self.paths_img3:
        #     self.paths_full = self.paths_img1 + self.paths_img2 + self.paths_img3   #10608+10608+10608
        if self.paths_img3:
            self.paths_full = self.paths_img1 + self.paths_img3   #10608+10608+10608
        # print(len(self.paths_img1))
        # print(len(self.paths_img2))
        # print(len(self.paths_full))
        # print(self.img_env1)
        # exit()
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            # choice = random.choice(['img1','img2','img3'])
            choice = random.choice(['img1','img3'])
            random_index = random.randint(0,len(self.paths_img1))
            if choice == 'img1':
                img1_path = self.paths_img1[index]
                img1 = util.read_img(self.img_env1, img1_path)               #hr
            # elif choice == 'img2':
            #     img1_path = self.paths_img2[index]
            #     img1 = util.read_img(self.img_env1, img1_path)                #srgan
            elif choice == 'img3':
                img1_path = self.paths_img3[index]
                img1 = util.read_img(self.img_env1, img1_path)   #srres
            
            img2_path = self.paths_full[random_index]     #hr
            img2 = util.read_img(self.img_env1, img2_path)
