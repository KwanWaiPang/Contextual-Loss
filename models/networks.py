...
def define_G(opt):
    ...
    if which_model == 'sr_resnet':
        ...
        netG = arch.RRDB_Net(in_nc=opt['in_nc'], out_nc=opt['out_nc'], nf=opt['nf'], \
            nb=23, upscale=opt['scale'], norm_type=opt['norm_type'], mode=opt['mode'],\
...
def define_E(opt):
    gpu_ids = opt['gpu_ids']
    opt = opt['network_E']
    which_model = opt['which_model_E']

    if which_model == 'discriminaotr_vgg_128_avgpool':
        netD = arch.Discriminaotr_VGG_128_AVGPOOL(in_nc=opt['in_nc'], base_nf=opt['nf'], \
            norm_type=opt['norm_type'], mode=opt['mode'] ,act_type=opt['act_type'])
    elif which_model == 'discriminaotr_vgg_128_avgpool2':
        netD = arch.Discriminaotr_VGG_128_AVGPOOL2(in_nc=opt['in_nc'], base_nf=opt['nf'], \
            norm_type=opt['norm_type'], mode=opt['mode'] ,act_type=opt['act_type'])
    elif which_model == 'Discriminaotr_VGG_128_AVGPOOL_Enhanced':
        netD = arch.Discriminaotr_VGG_128_AVGPOOL_Enhanced(in_nc=opt['in_nc'], base_nf=opt['nf'], \
            norm_type=opt['norm_type'], mode=opt['mode'] ,act_type=opt['act_type'])
    elif which_model == 'dis_acd':
        netD = sft_arch.ACD_VGG_BN_96()
